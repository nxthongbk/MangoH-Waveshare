...
static void set_memory_area(struct ws213fb_par *par, int x_start,
...
static void ws213fb_update_display(struct ws213fb_par *par)
{
	...
	u8 *vmem = par->info->screen_base;
 	...
 	//memcpy(ssbuf, vmem8,sizeof(ssbuf));
...
static int ws213fb_spi_probe(struct spi_device *spi)
{
	...
	dev_dbg(&spi->dev, "fb%d: %s frame buffer device,\n\tusing %d KiB of video memory\n",
